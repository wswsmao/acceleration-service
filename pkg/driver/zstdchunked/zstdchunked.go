...
func (d *Driver) Convert(ctx context.Context, p content.Provider, ref string) (*ocispec.Descriptor, error) {
	opts, docker2oci, err := getZstdChunkedConvertOpts(d.cfg)
	if err != nil {
		return nil, errors.Wrap(err, "parse zstdchunked conversion options")
	}
	image, err := p.Image(ctx, ref)
	if err != nil {
		return nil, errors.Wrap(err, "get source image")
	}
	return converter.DefaultIndexConvertFunc(zstdchunkedconvert.LayerConvertFunc(opts...), docker2oci, d.platformMC)(
...
func getZstdChunkedConvertOpts(cfg map[string]string) (opts []estargz.Option, docker2oci bool, err error) {
